...
## Property crime in each neighborhood order by their income 
```{r}
#| echo: false
...
crime_with_income <- crime_with_income %>%
  mutate(
    crime_type = case_when(
      ...
      str_detect(Crm.Cd.Desc, regex("Assault|Battery|Homicide", ignore_case = TRUE)) ~ "Violent Crime",
      TRUE ~ "Other"
    )
  )
property_crime_summary <- crime_with_income %>%
  filter(crime_type == "Property Crime") %>%  # Filter for property crimes
...
ggplot(property_crime_summary, aes(x = AREA.NAME, y = property_crimes)) +
  ...
  labs(
    title = "Property Crimes vs Median Income by Neighborhood",
...
```
A common assumption is that wealthier neighborhoods, given their higher-value properties and assets, may be more frequent targets for property crimes. However, when analyzing the relationship between property crime counts and median household income across neighborhoods (sorted in ascending order of income), the data presents a different perspective.

1.Higher Property Crime Rates in Lower-Income Neighborhoods:

Contrary to the assumption, neighborhoods with lower median incomes (left side of the graph) generally experience higher property crime counts. This trend suggests that property crimes may be more prevalent in economically disadvantaged areas rather than affluent ones.

2.Variation Among Neighborhoods with Similar Income Levels:

Even within similar income brackets, some neighborhoods experience significantly different property crime counts. For example, neighborhoods like Newton and Southeast have relatively lower property crime counts than other low-income neighborhoods, indicating that additional factors beyond income are influencing crime rates.

3.No Strong Pattern to Conclude the Assumption:

Although there is some indication of lower property crime rates in wealthier neighborhoods, the overall data does not reveal a strong or definitive pattern to support the assumption that richer neighborhoods are systematically targeted for property crimes. Variations across income levels highlight the complexity of factors influencing crime rates.

```{r}
#|echo: false
crime_summary$income_quartile <- cut(
  crime_summary$avg_income,
  breaks = quantile(crime_summary$avg_income, probs = seq(0, 1, by = 0.25), na.rm = TRUE),
  include.lowest = TRUE,
  labels = c("Low Income", "Lower Middle Income", "Upper Middle Income", "High Income")
)

ggplot(crime_summary, aes(x = income_quartile, y = total_crimes)) +
  geom_boxplot(aes(fill = income_quartile)) +
  scale_fill_viridis_d(name = "Income Quartile") +
  labs(
    title = "Distribution of Crime Counts by Income Quartile",
    x = "Income Quartile",
    y = "Total Crimes"
  ) +
  theme_minimal() +
...
```
There is a clear inverse relationship between income levesl and median crime counts, with lower income neighborhoods experienceing higher crime rates. However, variability within the low income quartile highlights that not all low-income heighborhoods are equally affected. Even in high income neighborhoods, a few outliners indicate that some wealthy areas still experience relatively high crime levels. High crime rates and variability in low-icome neighborhoods point to systemic issues like lack of economic opportunities or targeted policing. 
