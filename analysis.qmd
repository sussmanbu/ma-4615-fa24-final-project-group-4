---
title: Analysis
description: This dataset includes 2,500 crime cases per year from the LAPD (2020-2024), detailing crime type, location, time, and demographics. Our analysis focuses on identifying crime hotspots, peak activity periods, and correlations between crime rates and median income by area.
toc: true
draft: false
#runtime: shiny
---
...
# Introduction:
Crime rates can be interesting to analyze for policymakers, politicians, law enforcement or for students working on a data project. In this project, we wanted to look at the factors impacting crime, specifically under the LAPD's jurisdiction and looking at some variables that we thought might impact crimes. 
```{r Shiny map}
#| echo: false
#| message: false
#| warning: false
...
# Load data
lapd_boundary <- st_read(
  here("CityBoundaryofLosAngeles/geo_export_416eaeda-447a-4473-b003-37e2cad181ac.shp"),
  quiet = TRUE
) %>%
  st_make_valid() %>%
...
neighborhoods <- st_read(
  here("LA_Times_Neighborhood_Boundaries-shp/8494cd42-db48-4af1-a215-a2c8f61e96a22020328-1-621do0.x5yiu.shp"),
  quiet = TRUE
) %>%
  st_make_valid() %>%
  st_transform(crs = 4326)
...
# Define UI
ui <- fluidPage(
  titlePanel("Map of Los Angeles with LAPD Jurisdiction and Neighborhoods"),
...
server <- function(input, output, session) {
  output$map <- renderLeaflet({
    leaflet() %>%
      addTiles() %>%
      addPolygons(data = lapd_boundary, color = "black", weight = 2, fillOpacity = 0.1, group = "LAPD Boundary") %>%
      addPolygons(data = neighborhoods, color = "blue", weight = 1, fillOpacity = 0.2, group = "Neighborhoods") %>%
      addLayersControl(
        overlayGroups = c("LAPD Boundary", "Neighborhoods"),
